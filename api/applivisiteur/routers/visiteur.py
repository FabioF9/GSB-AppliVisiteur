...
def create_visiteur(request: schemas.Visiteur, db: Session = Depends(get_db)):
    new_user = models.Visiteur(VIS_NOM=request.VIS_NOM, VIS_ADRESSE=request.VIS_ADRESSE, VIS_CP=request.VIS_CP, VIS_VILLE=request.VIS_VILLE, LOG_LOGIN=request.LOG_LOGIN, LOG_MDP=Hash.bcrypt(request.LOG_MDP), SEC_CODE = 1, VIS_ADMIN = 1)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user 